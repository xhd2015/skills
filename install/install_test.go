...
func TestHandleInstallSkipsUnchangedSkillFile(t *testing.T) {
	...
	if strings.Contains(stdout, "create:") || strings.Contains(stdout, "update:") || strings.Contains(stdout, "delete:") {
		t.Fatalf("up-to-date install should not list inventory actions:\n%s", stdout)
	}
	content, err := os.ReadFile(skillFile)
	if err != nil {
		t.Fatalf("read skill file: %v", err)
