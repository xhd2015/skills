...
- `skill-cli/TOPIC.md` exists under the same skill directory (nested topics use
  TOPIC.md, not nested SKILL.md).
- Nested `skill-cli/SKILL.md` and legacy `topics/skill-cli.md` must not exist.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	nested := filepath.Join(skillDir, "skill-cli", "TOPIC.md")
	...
	if _, statErr := os.Stat(nested); statErr != nil {
		t.Fatalf("skill-cli/TOPIC.md missing at %s: %v", nested, statErr)
	}
	if _, statErr := os.Stat(nestedOldSkill); statErr == nil {
		t.Fatalf("skill-cli/SKILL.md must not be installed at %s", nestedOldSkill)
	}
